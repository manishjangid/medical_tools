...
for filen in files:
    ...
    if filen.endswith('.XLS'):
        data = pd.read_excel(filen, sheet_name = 'd-', header=None)
        data = data.append(pd.Series([np.nan]), ignore_index=True)
        data_frame_lastminus1.append(data)
        data = pd.read_excel(filen, sheet_name = 'd0', header=None)
        data = data.append(pd.Series([np.nan]), ignore_index=True)
        data_frame_last0.append(data)
        data = pd.read_excel(filen, sheet_name = 'd1', header=None)
        data = data.append(pd.Series([np.nan]), ignore_index=True)
        data_frame_last1.append(data)
        data = pd.read_excel(filen, sheet_name = 'd3', header=None)
        data = data.append(pd.Series([np.nan]), ignore_index=True)
